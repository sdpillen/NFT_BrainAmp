    ...
    # Marker class for storing marker information
        ...
        while not finish:
            ...
            if msgtype == 1:
                ...
                in the gigabytes.  if you want to reduce these filesizes, I recommend either implementing a downsampling 
                ...
                if NFT.Record == True:
                    for item in data:
                        ...
                        if counter == 32:
                            ...
                            elif NFT.LeftTag == True:
                                f.write('7')
                                print 'The target is now the LEFT side'
                                NFT.LeftTag = False
                            elif NFT.RightTag == True:
                ...
                '''We're getting back into SDK territory again. Watch your step. -SP'''
                if len(data1s) > channelCount * 1000000 / samplingInterval:
                    ...
                    what you see below: the 10th index of every 32nd index, or the 10th index of a given time series.  -SP'''
                    nowtime = time.time()
                    ...
                    NFT.SPTruVal = np.average(density[13:16])-np.average(density2[13:16]) # - np.average(density2[13:16])
                    NFT.LoNoise = density[1] #1 Hz
                    NFT.HiNoise = np.average(density[40:60]) #40-59 Hz
                    ...
                    NFT.SMRTimeSeries2 = SMRTimeSeries2
                    NFT.AlphaSeries = AlphaSeries
                    NFT.SMRDens = density[8:19]
                    NFT.SMRDens2 = density2[8:19]
                    NFT.AlphaDensity = densityA
                    #print 'stage is ', NFT.stage, ' ', NFT.pausetime
                    ...
                    if NFT.stage == 1 and NFT.pausetime == False:
...
class testGUI(wx.Frame): 
    ...
    def ToggleTMS(self, event):
        NFT.TMSFlag = not NFT.TMSFlag
        print 'TMS is set to ', NFT.TMSFlag
        self.buttonToggleTMS.SetLabel('TMS is ' + str(NFT.TMSFlag))
        self.sizer.Layout()
        self.Refresh()
        self.Update() 
        self.buttonConnect.Enable()
    ...
    def ToggleAlpha(self, event):
        NFT.alpha = not NFT.alpha
        print 'Alpha is set to ', NFT.alpha 
    ...
    def Rounds(self,event):
        if NFT.trialnumber == 60:
            NFT.trialnumber = 100
        else:
            NFT.trialnumber = 60
        self.buttonRounds.SetLabel(str(NFT.trialnumber) + ' Rounds')
        self.sizer.Layout()
        self.Refresh()        
    ...
    def Coords(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            NFT.pos = [0,0]
            if len(coords) == 2:
                NFT.pos[0] = int(coords[0])
                NFT.pos[1] = int(coords[1])
