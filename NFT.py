...
#These are flags used if the device is ever disconnected.  This should essentially never happen, but these are a holdover from the Emotiv days.
DISCONNECT = False
...
def reticle(color):
    pygame.draw.line(DISPLAYSURF, color, ((WINDOWWIDTH/2), WINDOWHEIGHT/2-40),((WINDOWWIDTH/2),WINDOWHEIGHT/2+40), (int(LINETHICKNESS*1.5)))
    pygame.draw.line(DISPLAYSURF, color, ((WINDOWWIDTH/2-40), WINDOWHEIGHT/2),((WINDOWWIDTH/2 + 40),WINDOWHEIGHT/2), (int(LINETHICKNESS*1.5)))
...
def Pausepoint(stage, score):
    ...
    if stage == 0:
        if time.time() < initialization: #if I don't make a 5 scond delay, things go funny
            ...
            if CONTROL == True or ControlRecording == True: resultSurf = SCOREFONT.render('BASELINE OR TMS LOCALIZATION', True, TEXTCOLOR)
            else: resultSurf = SCOREFONT.render('BASELINE OR TMS LOCALIZATION', True, TEXTCOLOR)
    if stage == 1:
...
def main():
    ...
    Phos.image = pygame.image.load("Phos.png").convert_alpha() #this formats the image into something compatible with the surface
    Phos.rect = Phos.image.get_rect()   #this gets details about the instruction sprite, such as size
    Phos.rect.center = [10000, 10000] #this puts the sprite far out of sight by default.  It moves when either defining it with mouse clicks, or by typing in the coordinates.
    ...
    NoPhos.image = pygame.image.load("NoPhos.png").convert_alpha()
    ...
    b.image = pygame.image.load("orb.png").convert_alpha() # Load the glider sprite
    ...
    while True:
        ...
        if moveflag == True:        
            if TargetVal < Threshold :   #This is a success state.
                ...
                b.image = pygame.image.load("orb.png").convert_alpha()
                #successflag = True 
        ...
        if LeftRightTimer < time.time():
            if LRPauseFlag == True:
                ...
                if len(resultarray) < 19:
                    ...
                    LeftFlag = not LRValues[(len(resultarray)-20)]  #The remaining rounds are pulled from the random list defined in the beginning.
